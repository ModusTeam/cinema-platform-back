...
public class UpdateMovieImagesCommandHandler(IApplicationDbContext context) 
    : IRequestHandler<UpdateMovieImagesCommand, Result<Guid>>
{
    public async Task<Result<Guid>> Handle(UpdateMovieImagesCommand request, CancellationToken ct)
    {
        ...
        movie.UpdateImages(request.PosterUrl, request.BackdropUrl, request.TrailerUrl);
